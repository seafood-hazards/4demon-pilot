...
- `4demon_pilot.sqlite` from the **latest** release
...
| `data-preparation.qmd` | How the raw 4Demon export was transformed before import |
...
longer exports a dataset file, and `data-export.qmd` was removed at v0.1.7.

## Git workflow

Gitflow (`main`, `develop`, `feature/*`, `release/*`). No pull requests — merge
branches directly with `--no-ff` and delete them when done.

## Releasing

**Every release must carry `4demon_pilot.sqlite` as an asset.** The site
...
5. **Create the GitHub release and upload the database, before pushing `main`:**
   ...
   gh release create vX.Y.Z --title vX.Y.Z --notes-from-tag \
     ../multised-engine/data/db/4demon_pilot.sqlite
   ```
