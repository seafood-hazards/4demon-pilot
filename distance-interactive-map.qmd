...
*   **Hover** over a point to quickly identify the **station** ID.
...
dist_order  = ["0 to 10km", "10 to 30km", "≥30km"]
dist_colors = ({
  "0 to 10km":   "#30123b",
  "10 to 30km":  "#28bbec",
  "≥30km":       "#fb8022"
...
points_web = db.query(`
  SELECT DISTINCT
    ...
    END AS Distance
  FROM sample s1
  JOIN station s2 ON s2.station_id = s1.station_id
  JOIN project p ON p.project_id = s1.project_id
`)
```
...

{
  ...
  if (!container._map) {
    ...
    for (const d of points_web) {
      ...
      marker.bindPopup(`
        <strong>Station ID:</strong> ${d.station_id}<br>
        <strong>Station Code:</strong> ${d.station_code}<br>
